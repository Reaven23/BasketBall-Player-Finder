class PagesController < ApplicationController
  ...
  def dashboard
    ...
    if params[:level].present?
      ...
      @questions = @games_played * 10
      @possible_points = ((@user.games.where(level: "easy").count * 100)) + ((@user.games.where(level: "medium").count * 300)) + ((@user.games.where(level: "hard").count * 500)) + ((@user.games.where(level: "legend").count * 1000))
